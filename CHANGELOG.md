...
## 2026-08-24 — 装備基盤の再構築とアドオン対応

艦と装備の能力値を、固定21項目の配列から`shincolle:atk_l`のような識別子ベースへ
作り直しました。本体を書き換えずに装備や能力値を追加できる基盤です。
既存ワールド・既存データパックはそのまま動きます。

### 追加

- **データパックだけで装備を追加**できるようになりました。任意のアイテムにJSONを
  1枚置けば艦の装備枠に入ります。入力補完用のJSON Schemaと、そのまま動く
  サンプルデータパックを同梱しています。
- **Javaアドオン向けの公開API**を本体に同梱しました。内部クラスに触れずに装備・
  能力値・命中時効果を追加できます。ビルド時にコンパイル検証されるサンプル付き。
- **KubeJS連携**を追加しました。スクリプトから艦の能力値そのものを新規登録できます。
  KubeJSが無い環境では読み込まれないため、導入は任意です。
- アドオンが追加した能力値も、艦の能力欄で最終値を確認できます(最大24件表示)。
- 設定`limitShipAttributesById`で、追加能力値にも上限を指定できるようにしました。

### 不具合修正

- **艦固有の攻撃デバフ24件を復元**しました。移植時に全24件が抜け落ちていたものです。
  長門の毒、金剛の弱体化、霧島の盲目、赤城・加賀の空腹、高雄・愛宕の移動速度低下、
  大和・レ級・戦艦棲姫の採掘速度低下など、味方13種・敵10種が対象です。
- 命中時に効果を適用する経路が原典11箇所に対し3箇所しか繋がっていなかったのを、
  近接・砲撃・艦載機・ミサイル・ビーム・連装砲・浮遊要塞へ配線し直しました。
- 離島棲姫の毒が発動しない問題を修正しました。原典どおり婚約し指輪効果を
  有効にした場合に発動します。
- 敵艦のCurios装備が再計算のたびに累積し、外しても残る問題を修正しました。
- データパックや設定に不正な記述があっても、その項目だけを拒否して
  サーバーが停止しないようにしました。理由はログへ個別に出力されます。

### 注意

- 通信プロトコルを更新したため、**旧バージョンのクライアントとは接続できません**。
  マルチプレイでは全員の更新が必要です(誤動作ではなく接続時に拒否されます)。

### 検証

- 自動テストを84件から158件へ拡張し、全件成功を確認しました。
- 専用サーバーと単体クライアントでの実機確認を実施しました。装備の着脱、
  艦固有デバフ、死亡卵からの復元、`/reload`時の再計算、optional MOD無しでの起動、
  旧バージョン接続の拒否、不正データパック・不正設定の隔離を確認済みです。
...
- 装備定義をアイテムID直書きの索引からResourceLocation主キーへ移行しました。アドオンから装備を追加・調整しやすくなります。
...
## 2026-08-21 — マウント騎乗・移動の実験的対応

マウント(空母棲姫・戦艦棲姫等の乗り物形態)まわりの実装を1.20.1向けに復元しました。**この節の内容は実機での動作確認がまだ済んでおらず、実験的な機能として扱ってください。**

- 騎乗操作の基礎実装(乗車、士気回復、所有者によるホスト艦GUI開閉、WASD移動・ジャンプ)を復元しました。
- クライアントでホスト艦より先にマウントの同期を受信した場合も、ホスト艦の生成後に関連付けを復元するようにしました。リログや追跡再開後にWASDで騎乗マウントが動かなくなる問題を修正します。
- マウントの移動方向がWASD入力・視点と逆になっていた問題を修正しました。
- マウント操縦中にインベントリキーを押すと、艦GUIと通常インベントリが同時に開いてしまう問題を修正しました。
- 複数人が同乗した場合の操縦者判定を修正しました。

## 2026-08-20 — 陣形移動・移動速度の修正

- 陣形移動の目標地点が壁・崖際・水中などの場合に、安全な地点へ補正するようにしました。
- 陣形を組めるのは5隻以上のチームに限定し、4隻以下では陣形バフを解除するようにしました。
- 陣形移動時の速度計算を原典と同じ「チーム最低速度+位置別加算」の式へ戻しました。
- 攻撃・護衛指示を、対象艦ごとに送信者との距離・次元を検証してから適用するようにしました。
- 装備・士気・ポーションの組み合わせで艦の移動速度が0になり完全停止する場合に、基礎速度の10%を下回らない下限を新設しました(原典から意図的に変更した仕様です)。

## 2026-08-19 — 指揮棒・造船所統合・大和ビームの修正

- 指揮棒で、ワールドスポーン地点付近への移動指示と、壁や艦体などで視線が通らない対象への攻撃・護衛指示を出せるようにしました。
- GROUPモードのツールチップで、選択中の艦を白、非選択を灰色で表示するようにしました。
- 小型・大型造船所の素材・燃料スロットを統一し、最大100万までの数値管理に変更しました。ホッパーやパイプ系MODからの自動搬入・搬出にも対応しました。
- 大和が2発目を発射した際、発射位置に留まらず実際に的へ向かって飛翔するようにしました。同じ対象への多重ダメージは発生しません。
- 大和のチャージ中(1発目〜2発目の間)に、艦体に電撃パーティクルと脈動する半透明の球体が纏うようにしました。
...
## 2026-08-18 — 造船所・大和・AI Goal間引きの修正

- 大型造船所の渦巻きエフェクトがちらつく問題を修正しました。
- 大和レーザーの色・太さが取り違っていた問題を修正しました。艦体サイズに応じた太さ・ピンク系の色になります。
- 雑魚敵艦(destroyer/cruiser等の量産艦)を撃破すると、スケールレベルに応じた確率でスポーン卵をドロップし、XPも得られるようにしました。
- 小型・大型造船所へ、バケツだけでなくパイプ等でも溶岩フルードを搬入できるようにしました。
- 大型造船所へ艦種スポーン卵を投入すると、解体材として素材ストックへリサイクルできるようにしました。
- AIのGoal内部処理(索敵、護衛、攻撃能力更新、経路再計算など)が、艦ごとのtickタイミングによっては一切動作しない問題を修正しました。複数艦を同時に出したときに一部の艦だけ反応が悪い・止まる原因になっていました。
