...
参照実装:
- 原典（upstream 1.10.2）: `EntityAIShipRangeTarget`
- 原典（upstream 1.10.2）: `BasicEntityShip`
...
以下は upstream 1.10.2 の `TargetHelper.Selector.apply(Entity target2)` の条件を上から順に詳しく分解したものです。
...
## 具体的なコード参照（抜粋と注釈）
以下は upstream 1.10.2 の `TargetHelper` の主要な判定箇所を抜粋し、それぞれ何をしているかを注釈したものです。
