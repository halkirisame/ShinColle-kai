...
MOD IDは `shincolle` のままなので、深これ向けのアドオンはそのまま利用できます。
ただし本家および移植版とは同一IDのため、同時には導入できません(置き換えになります)。
...
The mod id remains `shincolle`, so existing ShinColle addons keep working unchanged.
Because the id is shared, it replaces the original rather than running alongside it.
