        ...
        switch (kind) {
            case FRIENDLY_AUTOMATIC -> {
                if (entity.player()) {
                    ...
                    if (antiAirEligible || antiSubmarineEligible) {
                        if (antiAirEligible && policy.antiAir()
                                || antiSubmarineEligible && policy.antiSubmarine()) {
                            banned = banned(source, target, profiler);
                        }
                    } else if ((facts.friendlyShip() || facts.mount()) && policy.pvpFirst()) {
                        banned = banned(source, target, profiler);
                        if (!banned && facts.shipOwner()) {
                            allied = allied(source, target, profiler);
                        }
                    } else if (!facts.hostileShip()
