...
/**
 * Battleship Haruna entity.
 * model state: 0:equip, 1:head
 */
public class EntityBBHaruna extends BasicEntityShipSmall {

    public EntityBBHaruna(EntityType<? extends EntityBBHaruna> type, Level level) {
        super(type, level);
    ...
    public void calcShipAttributesAddEffect() {
        ...
        ShipInnateAttackEffects.put(this, ShipInnateAttackEffects.UNLUCK,
                level / 95, 100 + level, level);
    }
    ...
    public void aiStep() {
        ...
        if (!this.level().isClientSide()) {
            if (this.tickCount % 128 == 0) {
                // marriage ring aura: luck to nearby same-owner ships
                ...
                if (ownerUUID != null && getStateFlag(ID.F.IsMarried) && getStateFlag(ID.F.UseRingEffect) &&
                    ...
                    for (BasicEntityShip s : nearby) {
                        ...
                        if (sOwner != null && sOwner.equals(ownerUUID) && this.distanceToSqr(s) < 256.0D) {
                            s.addEffect(new MobEffectInstance(MobEffects.LUCK, dur, amp, false, false));
                        }
                    }
                }
            }
