...
/**
 * Destroyer Hibiki entity.
 * model state: 0:cannon, 1:armor, 2:hat1, 3:hat2, 4:hat3
 */
public class EntityDestroyerHibiki extends BasicEntityShipSmall implements IShipRiderType {

    private int riderType;

    public EntityDestroyerHibiki(EntityType<? extends EntityDestroyerHibiki> type, Level level) {
        super(type, level);
    ...
    public void aiStep() {
        ...
        if (!this.level().isClientSide()) {
            if (this.tickCount % 128 == 0) {
                // marriage aura: jump boost to owner
