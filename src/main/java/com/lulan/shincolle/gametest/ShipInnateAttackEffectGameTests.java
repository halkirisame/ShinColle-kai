...
public final class ShipInnateAttackEffectGameTests {
    ...
    public static void friendlyShipsRebuildAllOriginalInnateAttackEffects(GameTestHelper helper) {
        try (GameTestEntities entities = GameTestEntities.open(helper)) {
            verifyFriendly(helper, entities, ModEntities.BB_NAGATO.get(), ShipInnateAttackEffects.POISON, 2, 210);
            verifyFriendly(helper, entities, ModEntities.BB_RE.get(),
                    ShipInnateAttackEffects.MINING_FATIGUE, 1, 250);
            verifyFriendly(helper, entities, ModEntities.BB_YAMATO.get(),
                    ShipInnateAttackEffects.MINING_FATIGUE, 2, 250);
            verifyFriendly(helper, entities, ModEntities.BB_HARUNA.get(), ShipInnateAttackEffects.UNLUCK, 1, 250);
            verifyFriendly(helper, entities, ModEntities.BB_HIEI.get(), ShipInnateAttackEffects.POISON, 1, 230);
            verifyFriendly(helper, entities, ModEntities.BB_KIRISHIMA.get(),
                    ShipInnateAttackEffects.BLINDNESS, 0, 200);
            verifyFriendly(helper, entities, ModEntities.BB_KONGOU.get(), ShipInnateAttackEffects.WEAKNESS, 2, 250);
            verifyFriendly(helper, entities, ModEntities.CV_AKAGI.get(), ShipInnateAttackEffects.HUNGER, 1, 250);
            verifyFriendly(helper, entities, ModEntities.CV_KAGA.get(), ShipInnateAttackEffects.HUNGER, 1, 250);
            verifyFriendly(helper, entities, ModEntities.CA_ATAGO.get(), ShipInnateAttackEffects.SLOWNESS, 1, 250);
            verifyFriendly(helper, entities, ModEntities.CA_TAKAO.get(), ShipInnateAttackEffects.SLOWNESS, 1, 250);
            verifyFriendly(helper, entities, ModEntities.BB_HIME.get(),
                    ShipInnateAttackEffects.MINING_FATIGUE, 2, 250);

            BasicEntityShip isolated = createMarriedFriendly(
                    helper, entities, ModEntities.ISOLATED_HIME.get(), 150);
    ...
    public static void isolatedHimePoisonRequiresMarriageNotLevel(GameTestHelper helper) {
        try (GameTestEntities entities = GameTestEntities.open(helper)) {
            ...
            BasicEntityShip unmarried = createFriendly(helper, entities, ModEntities.ISOLATED_HIME.get(), 150);
            ...
            BasicEntityShip married = createMarriedFriendly(helper, entities, ModEntities.ISOLATED_HIME.get(), 74);
    ...
    public static void hostileShipsRebuildAllOriginalInnateAttackEffects(GameTestHelper helper) {
        try (GameTestEntities entities = GameTestEntities.open(helper)) {
            verifyHostile(helper, entities, ModEntities.BB_NAGATO_MOB.get(), ShipInnateAttackEffects.POISON,
                    0, 60, 1, 180);
            verifyHostile(helper, entities, ModEntities.BB_YAMATO_MOB.get(),
                    ShipInnateAttackEffects.MINING_FATIGUE, 0, 100, 2, 250);
            verifyHostile(helper, entities, ModEntities.BB_HARUNA_MOB.get(), ShipInnateAttackEffects.UNLUCK,
                    0, 100, 2, 250);
            verifyHostile(helper, entities, ModEntities.BB_HIEI_MOB.get(), ShipInnateAttackEffects.POISON,
                    0, 80, 2, 230);
            verifyHostile(helper, entities, ModEntities.BB_KIRISHIMA_MOB.get(),
                    ShipInnateAttackEffects.BLINDNESS, 0, 40, 2, 130);
            verifyHostile(helper, entities, ModEntities.BB_KONGOU_MOB.get(), ShipInnateAttackEffects.WEAKNESS,
                    0, 80, 2, 230);
            verifyHostile(helper, entities, ModEntities.CV_AKAGI_MOB.get(), ShipInnateAttackEffects.HUNGER,
                    0, 100, 1, 250);
            verifyHostile(helper, entities, ModEntities.CV_KAGA_MOB.get(), ShipInnateAttackEffects.HUNGER,
                    0, 100, 1, 250);
            verifyHostile(helper, entities, ModEntities.CA_ATAGO_MOB.get(), ShipInnateAttackEffects.SLOWNESS,
                    0, 100, 1, 250);
            verifyHostile(helper, entities, ModEntities.CA_TAKAO_MOB.get(), ShipInnateAttackEffects.SLOWNESS,
                    0, 100, 1, 250);
            helper.succeed();
        }
    }
    ...
    public static void onHitDispatcherUsesLiveEffectsAndExplicitProjectileSnapshot(GameTestHelper helper) {
        try (GameTestEntities entities = GameTestEntities.open(helper)) {
            BasicEntityShip source = createFriendly(helper, entities, ModEntities.BB_KONGOU.get(), 1);
            source.getAttackEffectMap().clear();
            ShipInnateAttackEffects.put(source, ShipInnateAttackEffects.POISON, 1, 120, 100);

            LivingEntity target = createTarget(helper, entities);
    ...
    private static void verifyFriendly(GameTestHelper helper, GameTestEntities entities,
                                       EntityType<?> type, ResourceLocation effectId,
                                       int amplifier, int duration) {
        BasicEntityShip ship = createFriendly(helper, entities, type, 150);
        assertEffect(ship, effectId, amplifier, duration, 100);
        assertEffectCount(ship, 1);
    }

    private static void verifyHostile(GameTestHelper helper, GameTestEntities entities,
                                      EntityType<?> type, ResourceLocation effectId,
                                      int scaleZeroAmplifier, int scaleZeroDuration,
                                      int scaleThreeAmplifier, int scaleThreeDuration) {
        BasicEntityShipHostile scaleZero = createHostile(helper, entities, type, 0);
        assertEffect(scaleZero, effectId, scaleZeroAmplifier, scaleZeroDuration, 25);
        assertEffectCount(scaleZero, 1);

        BasicEntityShipHostile scaleThree = createHostile(helper, entities, type, 3);
        assertEffect(scaleThree, effectId, scaleThreeAmplifier, scaleThreeDuration, 100);
        assertEffectCount(scaleThree, 1);
    }

    private static BasicEntityShip createFriendly(GameTestHelper helper, GameTestEntities entities,
                                                  EntityType<?> type, int level) {
    ...
    private static BasicEntityShip createMarriedFriendly(GameTestHelper helper, GameTestEntities entities,
                                                         EntityType<?> type, int level) {
        Entity entity = entities.add(type.create(helper.getLevel()));
        if (!(entity instanceof BasicEntityShip ship)) {
            throw new AssertionError("Failed to create friendly ship from " + type);
    ...
    private static BasicEntityShipHostile createHostile(GameTestHelper helper, GameTestEntities entities,
                                                        EntityType<?> type, int scale) {
        Entity entity = entities.add(type.create(helper.getLevel()));
    ...
    private static LivingEntity createTarget(GameTestHelper helper, GameTestEntities entities) {
        Entity entity = entities.add(EntityType.COW.create(helper.getLevel()));
