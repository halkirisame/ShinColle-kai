...
public final class TargetShadowGameTests {
    ...
    public static void disabledShadowDoesNotObserveOrCount(GameTestHelper helper) {
        waitForFixtureChunks(helper, () -> verifyDisabledShadowDoesNotObserveOrCount(helper));
    }

    private static void verifyDisabledShadowDoesNotObserveOrCount(GameTestHelper helper) {
    ...
    public static void enabledShadowMatchesFriendlyAndHostileSelection(GameTestHelper helper) {
        waitForFixtureChunks(helper, () -> verifyEnabledShadowMatchesFriendlyAndHostileSelection(helper));
    }

    private static void verifyEnabledShadowMatchesFriendlyAndHostileSelection(GameTestHelper helper) {
        TargetShadowComparison.resetCounters();
    ...
    public static void shadowThrowableCannotChangeLegacySelection(GameTestHelper helper) {
        waitForFixtureChunks(helper, () -> verifyShadowThrowableCannotChangeLegacySelection(helper));
    }

    private static void verifyShadowThrowableCannotChangeLegacySelection(GameTestHelper helper) {
        TargetShadowComparison.resetCounters();
        TargetShadowComparison.setEnabled(true);
        try (GameTestEntities entities = GameTestEntities.open(helper)) {
            Fixture f = new Fixture(helper, entities, false, 1, false);
    ...
    public static void shadowReplaysDrawWithoutChangingWorldRandomSequence(GameTestHelper helper) {
        waitForFixtureChunks(helper, () -> verifyShadowReplaysDrawWithoutChangingWorldRandomSequence(helper));
    }

    private static void verifyShadowReplaysDrawWithoutChangingWorldRandomSequence(GameTestHelper helper) {
        TargetShadowComparison.resetCounters();
        try {
            for (boolean hostile : new boolean[]{false, true}) {
    ...
    public static void randomSourceRejectsClientAndGoalSkipsClient(GameTestHelper helper) throws ReflectiveOperationException {
        waitForFixtureChunks(helper, () -> verifyRandomSourceRejectsClientAndGoalSkipsClient(helper));
    }

    private static void verifyRandomSourceRejectsClientAndGoalSkipsClient(GameTestHelper helper)
    ...
    public static void randomSourceChecksBoundsAndGoalRejectsInvalidDraws(GameTestHelper helper) throws ReflectiveOperationException {
        waitForFixtureChunks(helper, () -> verifyRandomSourceChecksBoundsAndGoalRejectsInvalidDraws(helper));
    }

    private static void verifyRandomSourceChecksBoundsAndGoalRejectsInvalidDraws(GameTestHelper helper)
            throws ReflectiveOperationException {
        try (GameTestEntities entities = GameTestEntities.open(helper)) {
            Fixture f = new Fixture(helper, entities, false, 3, false);
    ...
    private static void waitForFixtureChunks(GameTestHelper helper, FixtureVerification verification) {
        Vec3 sourcePosition = Fixture.sourcePosition(helper);
        Vec3 farthestEntityPosition = sourcePosition.add(6D, 0D, 0D);
        // These fixtures run spatial queries immediately after adding their entities.
        helper.startSequence().thenWaitUntil(() -> {
            helper.assertTrue(helper.getLevel().isPositionEntityTicking(BlockPos.containing(sourcePosition)),
    ...
    public static void shadowAddsOneQueryPerScanAndNoneDuringCooldown(GameTestHelper helper) {
        waitForFixtureChunks(helper, () -> verifyShadowAddsOneQueryPerScanAndNoneDuringCooldown(helper));
    }

    private static void verifyShadowAddsOneQueryPerScanAndNoneDuringCooldown(GameTestHelper helper) {
        TargetShadowComparison.resetCounters();
        TargetShadowComparison.setEnabled(true);
        try (GameTestEntities entities = GameTestEntities.open(helper)) {
            Fixture f = new Fixture(helper, entities, false, 3, false);
    ...
    public static void nonLivingOnlyScanRecordsMissingDrawWithoutConsumingRandom(GameTestHelper helper) {
        waitForFixtureChunks(helper, () -> verifyNonLivingOnlyScanRecordsMissingDrawWithoutConsumingRandom(helper));
    }

    private static void verifyNonLivingOnlyScanRecordsMissingDrawWithoutConsumingRandom(GameTestHelper helper) {
        TargetShadowComparison.resetCounters();
        TargetShadowComparison.setEnabled(true);
        try (GameTestEntities entities = GameTestEntities.open(helper)) {
    ...
    @FunctionalInterface
    private interface FixtureVerification {
